...
\subsection{Current Rupture Prediction Metrics}
...
\begin{table}[h]
\caption{Recent articles for aneurysm rupture prediction}
   \centering
   \resizebox{\textwidth}{!}{
\begin{tabular}{|l|p{40mm}|p{30mm}|c|p{25mm}|c|}
\hline
Author-Year & Parameters for \newline Analysis & Aneurysm \newline Location(s) & Ruptured IAs & Unruptured IAs& AUC or Accuracies \\
\hline
Qin et al., 2017\cite{qin2017morphological} & W, H, L, NW, Age, AR, Dmax, HW, BF, WSS, LSA, EL & MCA & 36 & 31 & 0.931 \\
&&&&&\\
Bijlenga et al., 2017\cite{bijlenga2017phases} &S, Age, Hyp, Race, SAH   & MCA, ICA, ACA/PcomA/Post & 598 & 243 & 0.681-0.756 \\
&&&&&\\
Detmer et al., 2018\cite{detmer2018external} &Age, Gender, S, OSIm, NSI & ACA, AcoM, BA, ICA, MCA, PCom, VA & 66 & 183 & 0.82 \\
&&&&&\\
Kocur et al., 2019\cite{kocur2019alternative}    &S, H, W, NS, AR, BF, H/W, SR, Aθ & AcoM, ICA, MCA, Post
 & 146 & 285 & 0.55-0.64 \\
 &&&&&\\
Wang et al., 2019\cite{wang2019simple} & R, DD, NW, W, D, Dia, AR, DW, BF, SR, Fθ, LDθ, SDθ,  & AcoM & 214 & 147 & 0.846   \\
 &&&&&\\
Varble et al., 2018\cite{varble2018stroke} & Age, SR, AR, UI, EL, NSI, WSS, OSI, WSSG, RRT, LSA, MWSS, PLc, EL & ACA, ICA, MCA, PcoM, Post & 102 (Train)  14 (Test) & 311 (Train) 115 (Test) & 0.767 \\
&&&&&\\
Jiang et al., 2018\cite{jiang2018novel} & L, H, AR, SR, UI, EI, Dθ, NSI, NWSSa, WSSm, WSSa, NWSSa, NWSSm, WSSG, LSA, OSI, RRT, Pm, Pa, NPa, NPm & ACA, AcoM, ICA, MCA, PcoM & 167 & 167 & 0.81 \\
\hline
 \multicolumn{6}{|p{240mm}|}{Anertieor communicating artery (AcoM), Anterior communicating artery (ACA), Internal carotid artery (ICA), ­­­Middle cerebral artery (MCA), Posterior communicating artery (PcoM), Posterior inferior cerebral artery (Post),  Aneurysm angle (Aθ), Area (A), Aspect Ratio (AR), Bottleneck factor (BF),  depth/width (DW), Diameter (D), Energy Loss (EL), Flow angle (Fθ), Height (H), Hypertension (Hyp), Irregular shape (IR), Large daughter artery angle (LDθ), Lateral angle ratio (LAR), Length (L), Low wall shear area (LSA), Mean diameter (MD), Mean wall shear stress (MWSS), Non-sphericity index (NSI), Normalized pressure average (NPa), Normalized pressure max (NPm), Normalized wall shear stress area (NWSSa), Normalized wall shear stress mean (NWSSm), Oscillatory shear index(max) (OSIm), Pressure average (Pa), Pressure loss(coefficient) (PLc), Pressure max (Pm), Relative residence time (RRT), Size ratio (SR), Small daughter artery angle (SDθ),  Surface (S), Temporally averaged WSS (TAWSS), Undulation Index (UI), Vessel angle (Vθ), Width (W), Wall shear stress (WSS), Wall shear stress gradient (WSSG), Wormsley number (WN), Wall shear stress mean (WSSm).}\\
 \hline
\end{tabular}}
...
\begin{subfigure}
  \centering
  \includegraphics[width=0.4\linewidth]{Wall_shear_stress_N_mm2.eps}
  \label{fig:sfig1}
\end{subfigure}
\begin{subfigure}
  \centering
  \includegraphics[width=0.4\linewidth]{Flow_Pattern_Vortex_in_mm_sec.eps}
  \label{fig:sfig2}
\end{subfigure}
...
\begin{figure}[h]
 \begin{center}
  \includegraphics[width=0.6\linewidth]{Cerebral_IA_locations.eps}
